    ...
    <section data-section id="values" className="relative">
      <div className="container-shell">
        <div className="grid md:grid-cols-2 gap-4 md:gap-6 relative">
          {VALUES.map((v) => (
            <div key={v.id} className="relative rounded-[4rem] p-10 min-h-[260px] flex flex-col items-center justify-center text-center font-display text-white bg-gradient-to-br from-[var(--color-brand-dark)] to-[var(--color-brand)] overflow-hidden">
              {/* Background Icon */}
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="bg-white/95 dark:bg-[var(--color-bg)]/95 border-2 border-[var(--color-brand-dark)] rounded-2xl px-6 py-4 text-center font-display text-lg md:text-2xl tracking-[0.15em] shadow-card max-w-[280px]">
              <span className="text-[var(--color-brand-dark)] dark:bg-gradient-to-br dark:from-[#5fffa1] dark:to-[#0d8f4a] dark:bg-clip-text dark:text-transparent font-bold">
